<template>
  <div class="bg-white w-full p-3 border border-gray-50 rounded-lg -lg">
    ...
    <div class="absolute inset-0">
      <!-- Background image -->
      <img
        v-if="challenge.photo.image"
        :src="challenge?.photo?.image"
        alt="Background Image"
        class="w-full h-full object-cover"
      />
      <img
        v-else
        src="@/assets/img/challenge-creativity.png"
        alt="Background Image"
        class="w-full h-full object-cover"
      />
    ...
    <!-- <div
      class="bg-[#FB3A82] p-6 rounded-lg mb-4 py-20 flex justify-between items-end relative"
    >
      <h2 class="text-2xl font-semibold text-white max-w-xs">
        {{ challenge?.name ?? "Nil" }}
      </h2>
    ...
    <div class="flex items-center mb-4">
      <h3 class="text-sm font-medium flex-grow">
        {{ challenge?.name ?? "Nil" }}
      </h3>
      <button
        class="bg-green-100 text-green-700 px-2 py-1.5 rounded-lg flex items-center text-xs gap-x-2"
      >
        {{ challenge?.challengeStatus ?? "Nil" }}
        <svg
          width="24"
          height="24"
          viewBox="0 0 24 24"
    ...
    <div class="flex justify-between items-center">
      <div class="flex items-center mb-4">
        <div class="flex items-center mr-4">
          <img
            src="@/assets/img/avatar-list.png"
            alt="Avatar"
            class="w-auto h-5"
          />
        </div>
        <span class="text-[#667085] text-xs"
          >{{ challenge?.participants?.length ?? "0" }} Participants</span
        >
      </div>
      <div>
        <button
          class="flex items-center gap-x-2 border rounded-full px-3 py-1.5 border-gray-100"
        >
          <svg
            width="16"
            height="16"
          ...
          >
            ...
            <g clip-path="url(#clip0_6586_1317)">
              <path
                d="M5.99528 11.9727C5.07359 11.5206 4.3318 10.7702 3.89035 9.84336C3.4489 8.91652 3.33371 7.86767 3.56349 6.86711C3.79327 5.86655 4.35453 4.97304 5.15613 4.33167C5.95772 3.6903 6.95259 3.33872 7.97918 3.33403C9.00578 3.32934 10.0038 3.67181 10.8112 4.30583C11.6187 4.93985 12.1881 5.82819 12.427 6.82661C12.6659 7.82503 12.5603 8.87489 12.1273 9.80573C11.6944 10.7366 10.9595 11.4937 10.0419 11.9542"
                stroke="#00CC99"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
              <path
                d="M9.73686 11.248L9.29443 12.2177L10.238 12.6688"
                stroke="#00CC99"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
              <path
                d="M8.29107 5.87072C8.56226 5.87072 8.78209 5.65088 8.78209 5.37969C8.78209 5.10851 8.56226 4.88867 8.29107 4.88867C8.01989 4.88867 7.80005 5.10851 7.80005 5.37969C7.80005 5.65088 8.01989 5.87072 8.29107 5.87072Z"
                stroke="#00CC99"
                stroke-width="0.8"
                stroke-linecap="round"
                stroke-linejoin="round"
            ...
            <defs>
              <clipPath id="clip0_6586_1317">
                <rect
                  ...
                  height="10.6667"
                  fill="white"
                  transform="translate(2.6665 2.66797)"
                />
              </clipPath>
            </defs>
          </svg>
          <p class="text-[#323232] text-xs font-medium">
            {{ challenge?.tag ?? "Nil" }}
          </p>
        </button>
      </div>
    ...
    <div
      class="grid grid-cols-4 gap-4 text-center mb-4 border border-gray-50 rounded py-3"
    >
      <div>
        <p class="font-medium text-[#667085] text-xs">Tasks</p>
        <p class="text-[#1D1F2C] text-xs">
          {{ challenge?.tasks?.length ?? "Nil" }}
        </p>
      </div>
      <div>
        <p class="font-medium text-[#667085] text-xs">Created</p>
        <p class="text-[#1D1F2C] text-xs">
          {{ moment.utc(challenge.createdAt).format("DD-MMM-YY") || "Nil" }}
        </p>
      </div>
      <div>
        <p class="font-medium text-[#667085] text-xs">Start Date</p>
        <p class="text-[#1D1F2C] text-xs">
          {{ moment.utc(challenge.startDate).format("DD-MMM-YY") || "Nil" }}
        </p>
      </div>
      <div>
        <p class="font-medium text-[#667085] text-xs">Due Date</p>
        <p class="text-[#1D1F2C] text-xs">
          {{ moment.utc(challenge.endDate).format("DD-MMM-YY") || "Nil" }}
        </p>
      </div>
    </div>
    <div class="mb-4">
      ...
      <ul class="space-y-6">
        <li class="flex items-center mb-2">
          <img
            src="@/assets/img/active-admin.png"
            alt="Jay Hargudson"
          ...
          <div class="flex-grow">
            ...
            <span
              class="text-white px-2 py-1 rounded-xl mt-1 text-xs bg-[#690571]"
              >Creator</span
            >
          </div>
            ...
            <button class="text-gray-500">
              ...
              >
                <g clip-path="url(#clip0_6586_1267)">
                  <path
                    d="M15.3333 2H4.66667C3.95942 2 3.28115 2.28095 2.78105 2.78105C2.28095 3.28115 2 3.95942 2 4.66667L2 12.6667C2 13.3739 2.28095 14.0522 2.78105 14.5523C3.28115 15.0524 3.95942 15.3333 4.66667 15.3333H6.6L9.56733 17.842C9.68771 17.9439 9.8403 17.9998 9.998 17.9998C10.1557 17.9998 10.3083 17.9439 10.4287 17.842L13.4 15.3333H15.3333C16.0406 15.3333 16.7189 15.0524 17.219 14.5523C17.719 14.0522 18 13.3739 18 12.6667V4.66667C18 3.95942 17.719 3.28115 17.219 2.78105C16.7189 2.28095 16.0406 2 15.3333 2V2ZM16.6667 12.6667C16.6667 13.0203 16.5262 13.3594 16.2761 13.6095C16.0261 13.8595 15.687 14 15.3333 14H13.4C13.0849 14.0001 12.7799 14.1118 12.5393 14.3153L10 16.46L7.462 14.3153C7.22104 14.1115 6.9156 13.9998 6.6 14H4.66667C4.31304 14 3.97391 13.8595 3.72386 13.6095C3.47381 13.3594 3.33333 13.0203 3.33333 12.6667V4.66667C3.33333 4.31304 3.47381 3.97391 3.72386 3.72386C3.97391 3.47381 4.31304 3.33333 4.66667 3.33333H15.3333C15.687 3.33333 16.0261 3.47381 16.2761 3.72386C16.5262 3.97391 16.6667 4.31304 16.6667 4.66667V12.6667Z"
                    fill="#858D9D"
                  />
                  <path
                    d="M6.66667 6.66732H10C10.1768 6.66732 10.3464 6.59708 10.4714 6.47205C10.5964 6.34703 10.6667 6.17746 10.6667 6.00065C10.6667 5.82384 10.5964 5.65427 10.4714 5.52925C10.3464 5.40422 10.1768 5.33398 10 5.33398H6.66667C6.48986 5.33398 6.32029 5.40422 6.19526 5.52925C6.07024 5.65427 6 5.82384 6 6.00065C6 6.17746 6.07024 6.34703 6.19526 6.47205C6.32029 6.59708 6.48986 6.66732 6.66667 6.66732Z"
                    fill="#858D9D"
                  />
                  <path
                    d="M13.3333 8H6.66667C6.48986 8 6.32029 8.07024 6.19526 8.19526C6.07024 8.32029 6 8.48985 6 8.66667C6 8.84348 6.07024 9.01304 6.19526 9.13807C6.32029 9.26309 6.48986 9.33333 6.66667 9.33333H13.3333C13.5101 9.33333 13.6797 9.26309 13.8047 9.13807C13.9298 9.01304 14 8.84348 14 8.66667C14 8.48985 13.9298 8.32029 13.8047 8.19526C13.6797 8.07024 13.5101 8 13.3333 8Z"
                    fill="#858D9D"
                  />
                  <path
                    d="M13.3333 10.666H6.66667C6.48986 10.666 6.32029 10.7363 6.19526 10.8613C6.07024 10.9863 6 11.1559 6 11.3327C6 11.5095 6.07024 11.6791 6.19526 11.8041C6.32029 11.9291 6.48986 11.9994 6.66667 11.9994H13.3333C13.5101 11.9994 13.6797 11.9291 13.8047 11.8041C13.9298 11.6791 14 11.5095 14 11.3327C14 11.1559 13.9298 10.9863 13.8047 10.8613C13.6797 10.7363 13.5101 10.666 13.3333 10.666Z"
                    fill="#858D9D"
                  />
                </g>
                <defs>
    ...
    <div class="flex justify-between gap-x-3 pt-6">
      ...
      <div class="w-full">
        <button
          @click="handleEditChallenge"
          class="bg-[#690571] text-white px-4 py-3 text-sm rounded-lg w-full"
        >
          Edit challenge
        </button>
      </div>
...
const handleEditChallenge = () => {
  // router.push(`/dashboard/challenge?id=${props.challenge.id}`)
  router.push({
    path: "/dashboard/challenge",
    query: { id: props?.challenge?.id },
  });
